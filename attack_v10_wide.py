#!/usr/bin/env python3
"""
Kyber768 Instruction Skip - V10 WIDE PARAMETER SEARCH
Much wider parameter space - maybe golden params have drifted
"""
...
# MUCH WIDER ranges - golden may have drifted
EXT_OFFSET_RANGE = range(5, 40)    # Was 15-19, now 5-39
WIDTH_RANGE = range(2, 12)          # Was 3-5, now 2-11  
OFFSET_RANGE = range(-25, 5)        # Was -15 to -9, now -25 to 4
...
def main():
    ...
    print(f"V10 WIDE PARAMETER SEARCH", flush=True)
    ...
    # Build all parameter combinations and shuffle for diversity
    ...
    try:
        for ext, w, off in all_params:
            ...
            if res == "SUCCESS":
                ...
                np.save(f"traces_repro/SUCCESS_v10_e{ext}_w{w}_o{off}.npy", trace)
                with open("KYBER_SUCCESS_V10.txt", "w") as f:
                    f.write(f"SUCCESS!\next={ext}\nwidth={w}\noffset={off}\nattempts={total}\nkey={key}\n")
            ...
            if total % 500 == 0:
                ...
                print(f"V10: {total} | Crash:{crashes}({pct:.1f}%) Norm:{normal} TO:{timeouts}", flush=True)
