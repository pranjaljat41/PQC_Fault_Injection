...
# 1. Width: Refining around the PERFECT 4.0%
WIDTH_OPTIONS = np.arange(3.8, 4.2, 0.05) 

# 2. Ext Offset: Locked to CLEAN SUCCESS location (20)
#    Absolute 20 -> Relative -75
EXT_OFFSET_RANGE = [-75]

FINE_OFFSET_RANGE = np.arange(-45.0, 45.0, 0.5)  # Standard Phase shift
                ...
                for w in WIDTH_OPTIONS:
                    for off in FINE_OFFSET_RANGE:
                        ...
                        if res == "different":
                            consecutive_timeouts = 0
                            different += 1
                            # Print EVERY different key to catch rare Gold occurrences
                            print(f"\n[DEBUG] Diff: {key}", flush=True)
