#!/usr/bin/env python3
"""
Kyber768 Instruction Skip Attack - V4 CLOCK GLITCH
Switch to clock glitching mode (clock_xor) instead of voltage glitching
Also widen ext_offset range significantly
"""
...
# WIDE ext_offset search with clock glitching
EXT_OFFSET_RANGE = range(0, 60)    # Very wide search
WIDTH_RANGE = range(2, 10)          # Moderate width
OFFSET_RANGE = range(-30, 0)        # Wide offset
...
def setup_scope_and_target():
    ...
    # CLOCK GLITCH MODE - Different from voltage!
    scope.glitch.clk_src = "clkgen"
    scope.glitch.output = "clock_xor"  # Clock glitch instead of voltage
...
def main():
    ...
    print("="*60, flush=True)
    print("ATTACK V4 - CLOCK GLITCH MODE", flush=True)
    ...
    print(f"glitch.output = clock_xor (CLOCK GLITCH)", flush=True)
    print("="*60, flush=True)
        ...
        for ext_offset in EXT_OFFSET_RANGE:
            for width in WIDTH_RANGE:
                for offset in OFFSET_RANGE:
                    for rep in range(3):  # 3 attempts per combo for speed
                        total += 1
                        ...
                        if result == "SUCCESS":
                            ...
                            np.save(f"traces_repro/SUCCESS_v4_clock_e{ext_offset}_w{width}_o{offset}.npy", trace)
                            with open("KYBER_SUCCESS_V4.txt", "w") as f:
                    ...
                    if total % 100 == 0:
                        ...
                        sys.stdout.write(f"\rV4-CLK: {total} | e={ext_offset},w={width},o={offset} | Crash:{crashes}({crash_pct:.0f}%) Norm:{normal} TO:{timeouts}")
                        sys.stdout.flush()
